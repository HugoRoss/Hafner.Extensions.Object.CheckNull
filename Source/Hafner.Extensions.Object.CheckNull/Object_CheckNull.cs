...
public static class ObjectExtension_CheckNull {
    ...
    public static T CheckNull<T>([NotNull] this T? obj, [CallerArgumentExpression(nameof(obj))] string paramName = "") where T : class {
        if (obj is null) {
            if (paramName is null) throw new ArgumentNullException("(unknown)", "The argument cannot be null!");
            if (paramName.IsExpression()) throw new ArgumentNullException(paramName, $"The result of expression '{paramName}' cannot be null!");
